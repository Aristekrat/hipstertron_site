    ...
    client.hgetall('resultSet', function(err, reply) {
        if (err || !reply) {
            ...
            switch (req.params.section) {
                case "beginning":
                    begin = reply.beginning;
                    end = reply.firstThird;
                    break;
                case "middle":
                    begin = reply.firstThird;
                    end = reply.secondThird;
                    break;
                case "end":
                    begin = reply.secondThird;
                    end = reply.end;
                    break;
            }
...
function sendConcertChunks(req, res, begin, end, oneThird) {
    ...
    var b = Number(begin); // It'd be good to find where the string conversion is happenning.
    var e = Number(end); // I think this code is good to leave in regardless though.
    for (var i = b; i < e; i++) {
